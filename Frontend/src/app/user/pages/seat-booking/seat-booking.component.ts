  ...
  private async loadSeats(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.log('No seats found, initializing automatically...');
        // Auto-initialize seats if they don't exist
        try {
          await this.http.post(`${environment.apiUrl}/seats/initialize`, {
            showtimeId: this.showtimeId,
            screenId: showtime.screen
          }).toPromise();
          // Retry loading seats after initialization
          bookedSeats = await this.http.get<any[]>(`${environment.apiUrl}/seats/showtime/${this.showtimeId}`).toPromise() || [];
          console.log('Seats initialized and loaded:', bookedSeats.length);
        } catch (initError) {
      ...
      const seats: Seat[] = screenConfig.seatMap
        ...
        .map((seatData: any) => {
          ...
          if (!dbSeat) {
            console.error(`No database seat found for ${seatLabel}. Seats not properly initialized.`);
            return null;
          }
          ...
          return {
            ...
            isHeld: dbSeat.isHeld || false,
            isDisabled: false
          };
        })
        .filter((seat: Seat | null) => seat !== null) as Seat[];

      if (seats.length === 0) {
        console.error('No valid seats found. Seats may not be properly initialized.');
        this.error.set('Seats not available. Please refresh the page.');
        return;
      }

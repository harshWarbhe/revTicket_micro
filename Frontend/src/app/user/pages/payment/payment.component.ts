...
export class PaymentComponent implements OnInit {
  ...
  private handlePaymentSuccess(response: any, draft: BookingDraft, breakdown: BookingCostBreakdown, contact: any): void {
    ...
    const verificationRequest = {
      ...
      showtime: new Date(draft.showDateTime).toISOString(),
      seats: draft.seats,
    ...
    this.razorpayService.verifyPayment(verificationRequest)
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          // Check if it's a booking service failure
          if (errorMsg.includes('booking') || errorMsg.includes('Booking')) {
            errorMsg = 'Payment successful but booking creation failed. Please contact support with your payment details.';
          }
          ...
          this.router.navigate(['/user/payment-failure'], {
            state: { 
              draft: draft,
              paymentSuccessful: errorMsg.includes('Payment successful')
            }
          });
        }
      });
